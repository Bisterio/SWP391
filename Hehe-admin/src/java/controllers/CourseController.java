...
public class CourseController extends HttpServlet {
    ...
    private final String REOPEN_COURSE = "ReopenCourse";
    private final String DELETE_COURSE_FOR_EDIT = "DeleteCourseEdit";
    private final String REOPEN_COURSE_FOR_EDIT = "ReopenCourseEdit";
                ...
                if (loginAuthor != null) {
                    ...
                    switch (action) {
                        case CREATE_COURSE: {
                            ...
                            CourseModel currentCourse = CourseDAO.createCourse(courseAuthorId, courseCategoryId, courseName, courseDescription, "Inactive", 0, courseDuration);
                        ...
                        case REOPEN_COURSE: {
                            int courseToReopenId = Integer.parseInt(request.getParameter("CourseToReopen"));
                            boolean result = CourseDAO.reactivateCourse(courseToReopenId);
                            if (result == true) {
                                url = AUTHOR_HOME_PAGE;
                            } else {
                                url = ERROR;
                        ...
                        case DELETE_COURSE_FOR_EDIT: {
                            int courseToDeleteId = Integer.parseInt(request.getParameter("CourseToDelete"));
                            boolean result = CourseDAO.deleteCourse(courseToDeleteId);
                            CourseModel currentCourse = CourseDAO.getCurrentCourse(courseToDeleteId);
                            request.setAttribute("CURRENT_COURSE", currentCourse);
                            if (currentCourse != null) {
                                url = EDIT_COURSE_PAGE;
                            } else {
                        ...
                        case REOPEN_COURSE_FOR_EDIT: {
                            int courseToReopenId = Integer.parseInt(request.getParameter("CourseToReopen"));
                            boolean result = CourseDAO.reactivateCourse(courseToReopenId);
                            CourseModel currentCourse = CourseDAO.getCurrentCourse(courseToReopenId);
                            request.setAttribute("CURRENT_COURSE", currentCourse);
                            if (currentCourse != null) {
                                url = EDIT_COURSE_PAGE;
                            } else {
