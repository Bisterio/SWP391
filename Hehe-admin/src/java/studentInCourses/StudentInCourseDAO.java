...
public class StudentInCourseDAO {
    ...
    public ArrayList<StudentModel> getStudentByCourse(int courseId) throws SQLException {
        ...
        try {
            conn = DBUtils.getConnection();
            if (conn != null) {
                ptm = conn.prepareStatement("SELECT s.Id as StudentId, s.FirstName, s.LastName, s.Status "
                        + "FROM Student s "
                        + "JOIN StudentInCourse sc "
